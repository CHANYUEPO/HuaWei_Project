...
void dijkstraPath(int dist[],int path[],int v0)
{
    ...
    for(i=0;i<N;i++)
    {
       if(Dijkstra_Matrix[v0][i]>0&&i!=v0)
        {
            dist[i]=Dijkstra_Matrix[v0][i];
            path[i]=v0;
        }
        else
        {
    ...
    for(i=1;i<N;i++)
    {
        ...
        for(k=0;k<N;k++)
        {
            if(visited[k]==false&&Dijkstra_Matrix[u][k]>0&&min+Dijkstra_Matrix[u][k]<dist[k])
            {
               dist[k]=min+Dijkstra_Matrix[u][k];
               path[k]=u;
            }
        }
    }
